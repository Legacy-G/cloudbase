...
# 3. Get credentials to connect kubectl
```
...
# 4. Deploy workloads (example: monitoring stack, odoo app)
```
kubectl apply -f prometheus-grafana.yaml -n monitoring
kubectl apply -f manifest.yaml -n odoo-prod
```
...
# run stress and load testing in powershell
```
k6 run login-test.js
```

# k6 stress testing linking with grafana
```
...
### Wipping out everything created


# Delete all known deployments, services, and pods in a namespace
```
kubectl delete all --all -n monitoring
kubectl delete all --all -n odoo-prod
```
